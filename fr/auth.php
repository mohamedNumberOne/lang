<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Lignes de langue d'authentification
    |--------------------------------------------------------------------------
    |
    | Les lignes de langue suivantes sont utilisées lors de l'authentification pour divers
    | messages que nous devons afficher à l'utilisateur. Vous êtes libre de modifier
    | ces lignes de langue en fonction des besoins de votre application.
    |
    */

    'failed' => "Ces informations d'identification ne correspondent pas à nos enregistrements.",
    'password' => "Le mot de passe fourni est incorrect.",
    'throttle' => "Trop de tentatives de connexion. Veuillez réessayer dans :seconds secondes.",
    'account_not_activated' => "Votre compte n'est pas activé.",
    "account_deleted" => "Le compte est supprimé, veuillez contacter le support",
    'no_user_found' => "Aucun utilisateur avec cette adresse e-mail n'appartient à notre système.",
    'pin_code' => "Le code PIN fourni est incorrect.",
];
